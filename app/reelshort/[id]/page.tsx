...
export default async function ReelShortDetailPage({ params }: PageProps) {
  const { id } = await params;
  const detail = await reelshort.getDetail(id);
    ...
    <div className="min-h-screen bg-[#141414] pb-12">
      ...
      <div className="relative w-full aspect-video md:aspect-[3/1] max-h-[500px]">
        ...
        <div className="absolute top-4 left-4 z-50">
           <Link href="/reelshort" className="flex items-center text-white hover:text-gray-300 transition-colors">
         ...
         {drama.episodes && drama.episodes.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
               {drama.episodes.map((ep, idx) => (
                  <Link 
                    key={idx} 
                    href={`/reelshort/watch/${id}/${ep.episodeNumber}`}
