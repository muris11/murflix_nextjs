...
import { SUBSCRIPTION_DURATION_LABELS, SubscriptionDuration, UserProfile, UserRole } from '@/types/auth';
    ...
    <div className="mx-auto max-w-2xl text-white">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-md bg-gray-800 p-4 border border-gray-700">
          <p className="text-sm font-medium text-gray-400">Subscription saat ini</p>
          <p className="mt-1 text-lg font-semibold">
            {user.role === 'admin' ? (
              <span className="text-primary">Admin (Tidak ada batas waktu)</span>
            ) : (
              <>
        ...
        {/* Account Status */}
        <div>
          <label className="block text-sm font-medium text-gray-300">Status Akun</label>
          <div className="mt-2 space-y-2 sm:flex sm:items-center sm:space-x-4 sm:space-y-0">
            <label className="flex items-center space-x-2 cursor-pointer">
              <input
                type="radio"
              ...
              >
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, subscription_duration: e.target.value as SubscriptionDuration | 'keep' })}
                    className="h-4 w-4 border-gray-300 text-primary focus:ring-primary"
                  />
                  <span className="ml-3 text-sm font-medium text-gray-200">Tidak Ubah</span>
                </div>
              </label>
